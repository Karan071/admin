...
import { OrgReview } from "@/data/Data";
...
export function Ngos() {
    return (
        <div className="flex gap-4 flex-col">
            <h1 className="text-2xl font-bold text-[var(--text-head)]">NGO's</h1>
            <OrgTable />
        </div>
    )
}
...
function OrgTable() {
    ...
    const [selectedStack, setSelectedStack] = useState<
        typeof OrgReview
    >(OrgReview[0] ? [OrgReview[0]] : []);
    const [focusedId, setFocusedId] = useState<number | null>(OrgReview[0]?.id || null);

    // Sorting logic
    const sortedData = [...OrgReview];
    if (sortConfig !== null) {
        sortedData.sort((a, b) => {
                        ...
                        <TableHeader className="bg-[var(--faded)] hover:bg-[var(--faded)] dark:bg-[var(--faded)] opacity-100">
                            <TableRow>
                                ...
                                <TableHead onClick={() => requestSort("organisation")} className="cursor-pointer text-[var(--text)]">
                                    NGO's {sortConfig?.key === "organisation" && (sortConfig.direction === "ascending" ? "↑" : "↓")}
                                </TableHead>
