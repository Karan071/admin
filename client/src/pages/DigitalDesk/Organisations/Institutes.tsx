...
import {  InstitutesTable } from "@/data/Data";
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-[var(--text-head)]">Institutes</h1>
        <StatsCards />
   <Topbar  />
    ...
    <div className="flex justify-between px-4 py-3 bg-[var(--background)] rounded-sm gap-4 border flex-wrap shadow-none">
      ...
      <div className="flex gap-4 align-middle">
         ...
         <Button variant="standard" size="new">
                  ...
                  <span className=""> Link References

</span>
                </Button>
         <Button variant="standard" size="new">
                  <Eye className="h-3 w-3" />
                  <span className="">Bulk Upload Entries</span>
                </Button>
                 <Button variant="standard" size="new">
                  <FileDown className="h-3 w-3" />
                  <span className="">Export Library Data</span>
                </Button>
    <Button
          variant="border"
...
function TableSection() {
  ...
  const [selectedStack, setSelectedStack] = useState<
    typeof InstitutesTable
  >(InstitutesTable[0] ? [InstitutesTable[0]] : []);
  const [focusedId, setFocusedId] = useState<string | null>(InstitutesTable[0]?.id || null);

  // Sorting logic
  const sortedData = [...InstitutesTable];
  if (sortConfig !== null) {
    sortedData.sort((a, b) => {
  ...
  const handleRowClick = (user: (typeof InstitutesTable)[0]) => {
            ...
            <TableBody className="overflow-visible relative z-0">
              {currentRecords.map((user) => (
                ...
                >
                  ...
                  <TableCell>
                    <div className="text-low">{user.InstituteType}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-low">{user.DataSource}</div>
                  </TableCell>
                   <TableCell>
                    <div className="text-low">{user.UpcomingUpdate}</div>
                  </TableCell>
