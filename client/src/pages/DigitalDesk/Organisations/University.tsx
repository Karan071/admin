...
import {UniversitiesTable } from "@/data/Data";
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-[var(--text-head)]">University</h1>
        <StatsCards />
   <Topbar  />
...
function TableSection() {
  ...
  >(UniversitiesTable[0] ? [UniversitiesTable[0]] : []);
  const [focusedId, setFocusedId] = useState<string | null>(UniversitiesTable[0]?.id || null);

  // Sorting logic
  const sortedData = [...UniversitiesTable];
  if (sortConfig !== null) {
    sortedData.sort((a, b) => {
  ...
  const handleRowClick = (user: (typeof UniversitiesTable)[0]) => {
