...
import { SchoolsTable} from "@/data/Data";
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-[var(--text-head)]">School</h1>
        <StatsCards />
   <Topbar  />
...
function TableSection() {
  ...
  const [selectedStack, setSelectedStack] = useState<
    typeof SchoolsTable
  >(SchoolsTable[0] ? [SchoolsTable[0]] : []);
  const [focusedId, setFocusedId] = useState<string | null>(SchoolsTable[0]?.id || null);

  // Sorting logic
  const sortedData = [...SchoolsTable];
  if (sortConfig !== null) {
    sortedData.sort((a, b) => {
  ...
  const handleRowClick = (user: (typeof SchoolsTable)[0]) => {
