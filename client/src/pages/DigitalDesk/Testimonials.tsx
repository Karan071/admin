import {  CircleArrowUp, CircleArrowDown, Search,  Users, FileCheck2, FileText, FileDown, BadgeQuestionMark, Plus } from "lucide-react";
...
function TestimonialsTable() {
  const dataWithIds = testimonialsData.map((item, index) => ({ ...item, id: index }));
  ...
  const toggleSelectAll = () => {
    if (selectedRows.length === currentRecords.length) {
      ...
      setSelectedRows(currentRecords.map(article => article.id));
    }
  };
